...
try{
    ...
    $stmt->execute();
    if($stmt->rowCount() == 0){
        echo json_encode(["success" => false, "message" => "ID de administrador no encontrado"]);
        exit;
    }
    ...
    if(!in_array($especial, [0, 1]) || !in_array($activa, [0, 1])){
        echo json_encode(["success" => false, "message" => "Valores de especial o activa no válidos."]);
        exit;
    }

    // ==================== Insertar datos de le la membresia ====================
    // Iniciar Transaction
    $pdo->beginTransaction();

    $sql = "INSERT INTO membresias (nombre, badge, precio, duracion_dias, descripcion, caracteristicas, descripcion_larga, especial, activa) 
            VALUES (:nombre, :badge, :precio, :duracion_dias, :descripcion, :caracteristicas, :descripcion_larga, :especial, :activa)";
    $stmt = $pdo->prepare($sql);
    ...
    if (!$result) {
        $pdo->rollBack();
        echo json_encode(["success" => true, "message" => "Error al registrar la membresía."]);
    }

    // Finalizacion de transaccion
    $pdo->commit();
    echo json_encode(["success" => true, "message" => "Membresía registrada exitosamente."]);
    exit;
